...
temp_player = Player(0, 1, "nickname", [5, 5], int(random.uniform(-1.4, 2.4)), [], [], 1)
...
def move_player(i):
    ...
    print(claimed_dots)
    if dir == 1:
        if x > 0 and [x - 1, y] not in claimed_dots:
            players[i].claimed_dots.append([x - 1, y])
            ...
            print("die")
    if dir == -1:
        if x < (sizeMap - 1) and [x + 1, y] not in claimed_dots:
            players[i].claimed_dots.append([x + 1, y])
            players[i].position[0] += 1
            print("down")
            arrMap[players[i].position[0]][players[i].position[1]] = players[i].id
            ...
            print("die")
    if dir == 0:
        if y > 0 and [x, y - 1] not in claimed_dots:
            players[i].claimed_dots.append([x, y - 1])
            ...
            print("die")
    if dir == 2:
        if y < (sizeMap - 1) and [x, y + 1] not in claimed_dots:
            players[i].claimed_dots.append([x, y + 1] )
            players[i].position[1] += 1
            print("right")
            arrMap[players[i].position[0]][players[i].position[1]] = players[i].id
